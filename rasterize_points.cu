...
RasterizeGaussiansCUDA(
    ...
    const torch::Tensor& unreliabilities,
    const torch::Tensor& loss_image,
    ...
    const bool debug)
{
  ...
  torch::Tensor out_minfo = torch::full({1, H, W}, 0.0, float_opts);
  torch::Tensor comp_unreli = torch::full({P, 4}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		unreliabilities.contiguous().data<float>(),
    loss_image.contiguous().data<float>(),
		...
		out_minfo.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		n_touched.contiguous().data<int>(),
    comp_unreli.contiguous().data<float>(),
        debug);
  }

  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, 
                        out_depth, out_opaticy, n_touched, 
                        out_minfo, comp_unreli);
}
